...
1. Go to **Developer Tools** → **Actions**
2. Select action: `ha_osc_control.list_endpoints`
3. Run the action
4. Check **Settings** → **System** → **Logs** to see all endpoint IDs

The endpoint ID format is: `{entry_id}_{osc_address_with_underscores}`

Example: entry `abc123` and OSC address `/mix/volume` becomes `abc123__mix_volume`
...
1. Go to **Developer Tools** → **Actions**
2. Select action: `ha_osc_control.add_button`
3. Fill in the form:
...
1. Go to **Developer Tools** → **Actions**
2. Select action: `ha_osc_control.add_slider`
3. Fill in the form:
...
```
Action: ha_osc_control.list_endpoints
```
...
5. Add actions using the OSC Control actions above
